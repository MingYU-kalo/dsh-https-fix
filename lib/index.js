...
export const Config = z.object({
  ...
  loginEnabled: z.boolean().default(true),                  // F11 启用登录(仅作用于插件 HTTPS 端口)
  loginUser: z.string().default(DEFAULT_USER),              // F12 登录账号(明文)
  loginPasswordHash: z.string().default(defaultPasswordHash()) // F13 登录密码(SHA-256 十六进制,不存明文)
})
...
const LOOPBACK_TARGET_LINES = [
  "isLoopback: transport?.ownsHost === true || pageLocation === void 0 || isLoopbackHostname(pageLocation.hostname)", // 0.1.2 ~ 0.1.5-rc.2(该行 rc.1 → rc.2 未变)
