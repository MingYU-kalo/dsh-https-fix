...
const TARGET_DSH_VERSION = "0.1.5-alpha.1"
...
export function apply(ctx, config) {
  ...
  function safeSource() {
    try {
      ...
      logger.warn(`[https-fix] 读取设置失败: ${err?.stack ?? String(err)}`)
      return entry
    }
  }
  ...
  function guard(label, fn) {
    try {
      ...
      if (result !== null && typeof result === "object" && typeof result.then === "function") {
        result.then(undefined, (err) => logger.warn(`[https-fix] ${label} 异常: ${err?.stack ?? String(err)}`))
      }
      return result
    } catch (err) {
      logger.warn(`[https-fix] ${label} 异常: ${err?.stack ?? String(err)}`)
      return undefined
    }
  }
  ...
  function startServer(cfg) {
    ...
    const fail = (err, what) => logger.warn(`[https-fix] ${what}异常: ${err?.stack ?? String(err)}`)
  ...
  function wantHttps(cfg) {
    const c = cfg ?? {}
    return c.enableHttps && !!c.domain && !!c.certPath && !!c.keyPath
  }
  ...
  ctx.inject(["settings"], (settingsCtx) => {
    guard("注册设置命名空间", () => settingsCtx.settings.installSection(ctx, NS, Config, entry, {
      setSource: (next) => { source = next },
      onChange: () => guard("设置变更同步", () => reconcile())
    }))
  })
  ...
  async function rpcResponse(endpoint, request) {
    ...
    try {
      ...
      logger.warn(`[https-fix] RPC ${endpoint} 异常: ${err?.stack ?? String(err)}`)
