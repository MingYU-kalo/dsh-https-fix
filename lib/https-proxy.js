...
function secureCookies(headers) {
  ...
  headers["set-cookie"] = list.map((cookie) => /;\s*secure\s*$/i.test(cookie) || /;\s*secure;/i.test(cookie) ? cookie : `${cookie}; Secure`)
  return headers
}
...
export function createHttpsProxy({ upstream, tokenProvider, log }) {
  return {
    handle(req, res) {
      ...
      const path = indexPathWithToken(req, tokenProvider)
      const isIndex = (req.url ?? "").split("?")[0] === "/" && req.method === "GET"
      const options = {
      ...
      const upstreamReq = httpRequest(options, (upstreamRes) => {
        if (isIndex && typeof log === "function") {
          log(`GET / cookie=${hasAuth ? "yes" : "no"} inject=${path === req.url ? "no" : "yes"} -> ${upstreamRes.statusCode}`)
        }
        res.writeHead(upstreamRes.statusCode ?? 502, secureCookies(upstreamRes.headers))
