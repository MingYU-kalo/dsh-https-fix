 ...
 * 校验走自有 RPC 通道 POST /api/https-fix/validate。
 */
    ...
    function HttpsFixCard(props) {
      ...
      async function validate() {
        ...
        try {
          ...
          const res = await fetch("/api/https-fix/validate", {
            method: "POST",
      ...
      return jsxs("li", { className: cx(css.card, open && css.cardOpen), children: [
        ...
        open ? jsxs("div", { className: css.body, children: [
          ...
          jsx(Field, { label: "域名 / IP", hint: "启用 https 时必填:域名或公网 IP 都行(没有域名就直接填 IP);用于 Host 透传与证书名字校验", children: [
            jsx(TextField, { value: cur("domain"), placeholder: "example.com 或 203.0.113.7", disabled: !writable, onChange: (v) => set("domain", v) })
          ] }),
          ...
          jsx(Field, { label: "TLS 证书(cert)路径", hint: "留空 = 插件自动生成自签证书(适合用 IP 访问、没有域名);填写则与 key 成对使用自有证书(PEM)", children: [
            jsx(TextField, { value: cur("certPath"), placeholder: "留空 = 自动自签;或 /path/to/fullchain.pem", disabled: !writable, onChange: (v) => set("certPath", v) })
          ] }),
          jsx(Field, { label: "TLS 密钥(key)路径", hint: "与证书成对填写;两者都留空时使用自动自签证书", children: [
            jsx(TextField, { value: cur("keyPath"), placeholder: "留空 = 自动自签;或 /path/to/privkey.pem", disabled: !writable, onChange: (v) => set("keyPath", v) })
          ] }),

          jsx(Field, { label: "自动模式(网页 token)", hint: "开启:自动获取 dsh 进程 token 并注入,浏览器直接访问 https://域名:端口 即可;关闭:不注入,需自行使用 dsh 启动时打印的带 token URL(等同原始 http 模式)", children: [
            jsx(BoolField, { label: "启用", value: cur("autoToken"), disabled: !writable, onChange: (v) => set("autoToken", v) })
          ] }),
          ...
          jsx(Field, { label: "客户端热补丁", hint: "一键给 dsh-client-connection 打 connection.isLoopback 豁免(按配置的域名/IP,端口无关),让经域名、IP 或前置代理访问时的设置页可用", children: [
