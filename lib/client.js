...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // cordis 的服务访问是"按 inject 白名单"的:只 inject "remote.settings" 不够 ——
    // 读 ctx.remote 本身就需要 inject "remote"(官方 ui-settings 也是两个都写)。
    // 漏了 "remote" 时浏览器里 ctx.remote.settings.describe() 直接抛
    // 'cannot get property "remote" without inject',卡片就永久停在"设置不可用"。
    ...
    function createController(ctx) {
      ...
      let snapshot = { status: "loading", writable: false, value: {}, user: {}, base: undefined, revision: undefined, mode: "host", error: undefined }
      let disposed = false
      let timer = null
      let attempt = 0
      let reported = false
      ...
      /**
       * 把浏览器侧的真实失败原因回传一份给宿主(落服务器日志)。
       * 卡片上只写得出"设置不可用",没有这句就只能靠猜 —— 只在首次失败时发一次。
       */
      function reportToHost(phase, message) {
        try {
          fetch("/api/https-fix/clientlog", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
              type: "client-request",
              method: "clientlog",
              rpcId: "c" + Math.random().toString(36).slice(2),
              payload: { where: phase, message: String(message).slice(0, 400) }
            })
          }).catch(() => {})
        } catch { /* fetch 不可用就算了,不能因为诊断把卡片弄挂 */ }
      }

      /** 失败后的退避重试:1s→2s→4s→8s→16s→30s 封顶,直到读到为止。 */
      function schedule() {
      ...
      function fail(message) {
        // 已经读到过就别把好数据降级成"不可用",只标注这次同步失败。
        snapshot = loaded
          ? Object.assign({}, snapshot, { error: message })
          : Object.assign({}, snapshot, { status: "unavailable", error: message })
        if (!reported) { reported = true; reportToHost("settings.describe", message) }
        emit()
        schedule()
      }

      /** 写设置:带最近一次读到的 revision,写完重读。失败抛给调用方(卡片显示原因)。 */
      async function write(ops) {
        ...
        if (!response || response.ok !== true) {
          const message = response && response.error ? (response.error.message || response.error.code || "写入设置失败") : "写入设置失败"
      ...
      async function refresh() {
        try {
          ...
          if (disposed) return
          if (!response || response.ok !== true) {
            fail(response && response.error ? (response.error.message || response.error.code || "describe 失败") : "describe 没有返回")
            return
          }
          const view = response.value || {}
          const row = (view.namespaces || []).find((item) => item && item.ns === NS)
          if (!row) {
            fail("宿主没有登记 https-fix 设置命名空间(插件未激活或没有 volatile 字段)")
            return
          }
          loaded = true
          attempt = 0
      ...
      // 与官方 settings mirror 同一套失效信号:设置文档变了、连接重建了、页面重新可见
      // 都重读一次。官方会重试,我们以前只读一次,失败一次就永久"设置不可用"。
      ctx.effect(() => {
        const offs = []
        try {
          if (ctx.remote && typeof ctx.remote.$on === "function") offs.push(ctx.remote.$on("settings/document-updated", reload))
          if (typeof ctx.on === "function") offs.push(ctx.on("connection/reset", reload))
          if (typeof document !== "undefined" && typeof document.addEventListener === "function") {
            const onVisible = () => { if (document.visibilityState === "visible") reload() }
            document.addEventListener("visibilitychange", onVisible)
            offs.push(() => document.removeEventListener("visibilitychange", onVisible))
          }
        } catch { /* 事件总线可用性因版本而异,挂了也不影响首读 */ }
        return () => { for (const off of offs) { try { off() } catch {} } }
      }, "https-fix: settings refresh")
    ...
    function HttpsFixCard(props) {
      ...
      const syncError = typeof snap.error === "string" && snap.error ? snap.error : "";
      ...
      return jsxs("div", { className: cx(css.card, open && css.cardOpen), children: [
        ...
        open ? jsxs("div", { className: css.body, children: [
          ...
          !writable || unavailable ? jsx("p", { className: css.readOnly, role: "status", children: unavailable ? "设置不可用(需经回环地址或受信域名访问)" + (syncError ? ":" + syncError : "") : "只读:当前会话没有写入权限" }) : null,
          !unavailable && syncError ? jsx("p", { className: css.warn, role: "status", children: "最近一次同步失败(仍显示上次读到的值):" + syncError }) : null,
